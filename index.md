...
<script setup>
  ...
  // Добавляйте новых участников в нужный список по аналогии с существующими.
  const developers = [
    {
      avatar: 'https://avatars.githubusercontent.com/u/169675635?v=4',
      name: 'EHqador (Он же Никитка)',
      title: 'Создатель этой Вики',
      links: [{ url: 'https://github.com/NoMorelov3', type: 'github' }]
    },
    {
...
<div class="main-content-layout">
  ...
  <div class="custom-team-section">
    ...
    <div class="team-centered-wrapper">
      <div v-for="member in developers" :key="member.name" class="custom-member-card">
        <img class="member-avatar" :src="member.avatar" :alt="member.name">
        <div class="member-name">{{ member.name }}</div>
        <div class="member-title">{{ member.title }}</div>
        <div class="member-links">
          <a v-for="link in member.links" :key="link.url" :href="link.url" target="_blank" rel="noopener" class="member-icon-link">
            <img class="icon-svg" :src="`https://simpleicons.org/icons/${link.type}.svg`" :alt="link.type">
          </a>
        </div>
      </div>
    </div>
  ...
  <div class="custom-team-section">
    ...
    <div class="team-centered-wrapper">
      <div v-for="member in supporters" :key="member.name" class="custom-member-card">
        <img class="member-avatar" :src="member.avatar" :alt="member.name">
        <div class="member-name">{{ member.name }}</div>
        <div class="member-title">{{ member.title }}</div>
        <div class="member-links">
